      ...
      for (const st of stops) {
        if (st.latitude && st.longitude && st.geo_fence_radius) {
          ...
          if (!wasInside && inside) {
            ...
            // --- Send POST request to external API when entering geofence ---
            const domainName = activeShipment.domain_name; // fallback if not present
            ...
            try {
              ...
              console.error('❌ Failed to notify external API:', (err && (err as any).response && (err as any).response.data) || err?.message || err);
            }
          }
