...
import { 
    shipment, equipment, group, group_entity, user_group, entity,  
...
import { eq, inArray, and, gte, lt, desc, sql  } from "drizzle-orm";
  ...
  try {
    ...
    for (const stop of stopsRows) {
      if (stop.entry_time && stop.exit_time) {
        const entryTime = new Date(stop.entry_time).getTime();
        const exitTime = new Date(stop.exit_time).getTime();
        const ceta = Math.round((exitTime - entryTime) / 1000 / 60); // in minutes
        stop.ceta = ceta.toString();
      } else {
        stop.ceta = "0";
      }
      if (stop.geta) {
